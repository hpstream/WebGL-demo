...
import { Camera, Matrix4, PerspectiveCamera, OrthographicCamera, Spherical, Vector2, Vector3 } from "three"
...
export class OrbitControls {
  ...
  pointerdown({ clientX, clientY, button }: PointerEvent) {
  ...
  dollyPerspectiveCamera(dollyScale: number) {
    this.spherical.radius /= dollyScale
  }
  dollyOrthographicCamera(dollyScale: number) {
    const { camera } = this
  ...
  panPerspectiveCamera({ x, y }: Vector2) {
    const {
      camera,
      dom: { clientHeight },
      panOffset, screenSpacePanning, target
    } = this
    const { matrix, position, fov, up } = camera as PerspectiveCamera;
  ...
  panOrthographicCamera({ x, y }: Vector2) {
    const {
      camera,
      dom: { clientWidth, clientHeight },
      panOffset, screenSpacePanning
    } = this
    const { right, left, top, bottom, matrix, up } = camera as OrthographicCamera;
